...
@
\subsection{3B}
Here I observed that a vector can take a matrix as a subset. Thus I creates a matrix with 1001 as the default element. That is becuase I want to add a zero at the end of muA in the final step so that I won't worry about the problem of the length of the vector after "subseting". 
<<>>=
...
result_CaseA<-colSums(wgA_mat*c(muA,0)[IDAmatrix])
head(result_CaseA)
@
\subsection{3C}
...
@
\section{3D}
By looking at the result from R microbenchmarking, we can observe that it takes about 300 milliseconds in average to run sapply for both CaseA and CaseB, because it essentially calls a for loop in C, which is by any means a "stepwise" calculation. However, after vectorizing the calculation for Case A and B. Case A spends about 20 milliseconds, which means about one order magnitude speed up. Case B spends about 2-3 miliseconds, which means about two order magnitude speed up.
\newline
<<>>=
...
microbenchmark(result_A_A<-sapply(1:length(IDsA)
               ...
               result_CaseA<-colSums(wgA_mat*c(muA,0)[IDAmatrix]),times=10L)
...
By the command internal inspect, I found that the computer also stores the names of y as an attribute in the RAM, as it's shown in the code output. By printing the object size of name attributes of y, we found that names occupied 56 MB in the memory.\\
