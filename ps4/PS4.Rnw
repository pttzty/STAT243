...
@
In this section, we will eliminate any loops or apply functions in our function. Thus we set up a vector from 1 to n minus one, and do the arithematic operations toward this vector.
\newline
<<cache=TRUE>>=
